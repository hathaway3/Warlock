...
*   **🗑️ Removed (breaking):**
    *   **The legacy EJS UI.** The server-rendered pages (`/dashboard`, `/hosts`, `/login`, `/install`, `/settings`, `/2fa-setup`, `/host/*`, `/service/*`, `/application/*`, `/files`, `/test`), their route files, all 39 EJS templates but one, ~400 legacy asset files under `public/assets/` (CodeMirror 5, Font Awesome, page scripts, web components), and the `USE_LEGACY_UI` environment variable are gone. The React SPA is the only UI; install, login, and 2FA setup are handled by it via `/api/auth/*`. Anyone who was running with `USE_LEGACY_UI=true` must move to the SPA. The REST/SSE API under `/api/*` is unchanged. `public/assets/media/` (game artwork) is kept.
    *   `views/error.ejs` is retained as the only server-rendered page (the fallback for non-API errors) and was rewritten to be self-contained, with no dependency on the removed assets.
*   **Added:**
    *   `npm run release -- <patch|minor|major|X.Y.Z> [--dry-run]` cuts a release: bumps the version in all four `package*.json` files, rolls `[Unreleased]` under a dated heading, commits, and tags (never pushes). Also syncs the root `package-lock.json`, which had drifted to 1.2.2.
    *   Every push to `main` now tags the Docker Hub image `<version>-dev.<commit count>` (e.g. `1.3.0-dev.412`) in addition to `dev`.
*   **Fixed:**
